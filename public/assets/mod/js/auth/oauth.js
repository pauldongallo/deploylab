var ROOTURL = "http://mabuhayflowers.test";
var siteDomain = location.origin;
...
const NOTESROUTE = RESTROOT + '/wp/v2/notes';
...
var jso = new JSO({
    ...
    redirect_uri: siteDomain + "/public/admin/orders/orders-dashboard.php", // The URL where you is redirected back, and where you perform run the callback() function.
...
function oauthLogin(){
	jso.getToken();
	// console.log("what");
	var newtoken = jso.getToken(function(newToken){
...
$('#logout').click(function() {
	alert("logout");
    oauthLogout();
    // window.location.href = "http://mabuhayflowers.test/wp-json/mod/v1/logout";
	// window.location.href = "http://modin.test";
});

// (function() {
// 	// If we are on the home page, redirect to tasklist.html:
// 	if ( location.pathname == "/admin/login.php" ) {
...
// 			location.href = "/admin/sign-in.php";
